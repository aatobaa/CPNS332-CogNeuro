...
%%
% Extract relevant features, which are basically frequency bands averaged
% over time. 
...
%Just counting the number of inputs broken out of each sound wave. 
k1s = size(kira1_c,2);
k2s = size(kira2_c,2);
k3s = size(kira3_c,2);
...
%% Creating the Neural Network - Backpropagation learning rule
...
%variables are named as "inp_input" meaning input to the input layer,
%"inp_hidden" meaning input to the hidden layer, "net_input_h" meaning net
%input to the hidden layer, etc. 
...
%Transfer function derivative below assumes sigmoid transfer fxn with alpha of -1;
...
for j = 1:numEpochs
    ...
    % Calculate the error from this epoch. 
...
inp_hidden = output_h; 
net_input_o = Wo*inp_hidden + bo; 
voiceClassification = transfer_fn(net_input_o,-1);
plot(voiceClassification,'.')
% Average back features per sound bite

%TODO: Give a final classification of each sound bite
%TODO: Make plots to show that it classifies reliably. 

voiceClassification(1:k1s)
voiceClassification(1:k1s)
voiceClassification(1:k1s)

voiceClassification(1:p1s)
voiceClassification(1:p1s)
voiceClassification(1:p1s)

finalClassification = [mean(voiceClassification(1:k1s)) mean(voiceClassification(1+k1s:)) mean(voiceClassification(1:k1s)) mean(voiceClassification(1:k1s)) mean(voiceClassification(1:k1s)) mean(voiceClassification(1:k1s))]
